...
| avoid_linestring.types                      | [-]   | string array | linestring types in the lanelet maps that will not be crossed when expanding the drivable area; the format "type.subtype" is supported to also account for the linestring subtype | ["road_border", "curbstone"] |
